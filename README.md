...
해시맵은 Python `dict` 대신 직접 구현한 체이닝 해시맵을 사용합니다. 해시 함수는 문자열 키의 각 문자를 왼쪽부터 순회하며 다음 식으로 64비트 값을 누적합니다.

```text
hash = (hash * 131 + ord(character)) mod 2^64
```

`131`은 문자열 해싱에서 자주 쓰는 작은 소수 계열 multiplier입니다. 앞 문자와 뒤 문자의 순서가 해시값에 반영되고, 계산이 단순해 CLI 과제 규모에서 충분히 빠릅니다. 충돌이 나면 같은 버킷의 이중 연결 리스트에 엔트리를 연결하는 체이닝 방식으로 해결하며, 로드 팩터가 `0.75`를 초과하면 버킷 수를 2배로 늘려 재배치합니다.
